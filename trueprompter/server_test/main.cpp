    ...
    void Run(uint16_t port) {
        ...
        try {
            ...
            server.set_open_handler([&server, this](websocketpp::connection_hdl hdl) {
                auto recognizer = std::make_shared<NRecognition::TContextualRecognizer>(
                ...
                auto matcher = std::make_shared<NRecognition::TOnlineMatcher>(
                    std::make_shared<NRecognition::TDefaultMatcher>(NRecognition::TDefaultMatcher::EMode::WeightedPhonemes),
                    recognizer->GetSampleRate() * 3.0f / recognizer->GetFrameSize()
                );
                Clients_.emplace(
                    hdl,
