...
Recognizer::Recognizer(Model *model, float sample_frequency) : model_(model), sample_frequency_(sample_frequency) {
...
const char* Recognizer::PartialResult()
{
    ...
    if (partial_words_) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (model_->word_syms_->Find(words[i]) == "привет") {
                found = true;
            }
        ...
        if (size) {
            std::cout << "========" << std::endl;
            for (auto& w : words) {
                std::cout << model_->word_syms_->Find(w) << " ";
            }
            std::cout << std::endl;
            auto opts = kaldi::PhoneAlignLatticeOptions();
            opts.replace_output_symbols = true;
            kaldi::PhoneAlignLattice(clat, *model_->trans_model_, opts, &clatCopy);

            {
                auto c = clatCopy;
                fst::ScaleLattice(fst::LatticeScale(0.0, 1.0), &c);
                MinimumBayesRisk mbr(c);
                const vector<int32>& phones = mbr.GetOneBest();
                //kaldi::ConvertCompactLatticeToPhones(*model_->trans_model_, &clatCopy);

                for (auto p: phones) {
                    std::cout << model_->phone_syms_->Find(p) << " ";
                }
                std::cout << std::endl;
            }
            {
                MinimumBayesRisk mbr(clatCopy);
                const vector<int32>& phones = mbr.GetOneBest();
                //kaldi::ConvertCompactLatticeToPhones(*model_->trans_model_, &clatCopy);
